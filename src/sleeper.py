...
def get_transactions_by_week(week: int = 1):
    ...
    for trade in r.json():
        ...
        added = (
            trade["adds"] if trade["adds"] is None else list(trade["adds"].keys())[0]
        )
        dropped = (
            trade["drops"] if trade["drops"] is None else list(trade["drops"].keys())[0]
        )
    ...
    for r in response.json():
        ...
        roster = Roster(
            ...
            points_against=float(
                f"{stats['fpts_against']}.{stats['fpts_against_decimal']}"
            ),
...
def get_projected_scores(roster: Roster):
    ...
    projections = (
        get_player_projected_scores()
    )  # TODO: Really shouldn't call this for every manager
    ...
    def apply_scoring(score_settings, _stats):
        ...
        if (
            stats.get("fga", 0) > 0.1
        ):  # Attempting more than .1 field goals means they are a kicker
            # TODO: Kickers still don't align perfectly, but it's close enough I'm over it for now
            _score += (
                score_settings.get("fgmiss", 0) * (stats["fga"] - stats["fgm"]) * 0
            )
            _score += (
                score_settings.get("xpmiss", 0) * (stats["xpa"] - stats["xpm"]) * 0
    ...
    for player_id in roster.starters:
        ...
        if in_game:
            ...
            player_current_score = apply_scoring(
                scoring_settings, player_stats[player_id]["stats"]
            )
    ...
    for player_id in roster.starters:
        ...
        try:
            ...
            print(
                f"KeyError: {player['player_id']} has no attribute pts_half_ppr for manager {roster.manager_id}"
            )
